...
export default function CategoriasModal({ modalType, showModalState, setShowModalState }: CategoriasModalProps) {
  const [inputCodigo, setInputCodigo] = useState("");
  const [inputNombre, setInputNombre] = useState("");

  const obtieneCategorias = async () => {
    return {
      data: [{ CODIGO: "00001", NOMBRE: "Supervisor" }],
    };
  };
  ...
  return (
    <>
      {showModalState && modalType === "categoriasModal" ? (
        <DraggableModal
          title="Categorías"
          isVisible={showModalState && modalType === "categoriasModal"}
